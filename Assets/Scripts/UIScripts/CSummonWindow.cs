    ...
    private void Update()
    {
        ...
        if (!_bCheck1 && _timer >= 0)
        {
            ...
            if (scale < 1)
            {
                ...
                isEnd = true;
                if(!_isSkip)
                    StartCoroutine(ImgShaking(imgBase.transform, 0.5f, 5f, 0.2f));
            }
        ...
        if (_bCheckFin && _timer >= 0 && !_bAppear)
        {
            ...
            if (!_isImgLoad)
            {
                ...
                if (!_isSkip)
                {
                    _playingCoroutine = ImgShaking(streamerImg.transform, -0.4f, 5f, 2.0f);
            ...
            if (color.a < 1 && !_isPhaseEnd)
            {
                ...
                if (color.a >= 1)
                {
                    ...
                    {
                        _timer = -1.5f;
                        _playingCoroutine = ImgShaking(streamerImg.transform, 0.5f, 10f, 1f);
                        StartCoroutine(_playingCoroutine);
                    }
                }
            }
